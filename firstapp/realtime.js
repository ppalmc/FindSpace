...
route.get('/recommWS', async (req,res) => {
    try {
        const current_lat = 0;
        const current_long = 0;
        const inrangeWS = []
        const allWorkspace = await (await pool.query("SELECT R1.workspaceid, R2.workspaceid, R2.ws_lat, R2.ws_long, R1.ppl_in_WS, R2.totalseat, R1.ppl_in_WS/R2.totalseat AS crowdedness, CASE WHEN R1.ppl_in_WS/R2.totalseat<=0.25 THEN 1 WHEN R1.ppl_in_WS/R2.totalseat>0.25 AND R1.ppl_in_WS/R2.totalseat <= 0.5 THEN 3 ELSE 5 END AS crowdednessStatus FROM ( SELECT DISTINCT workspaceid, SUM(H.num_in_out) AS ppl_in_WS FROM hardware H GROUP BY workspaceid) AS R1,( SELECT DISTINCT * FROM workspace WS) AS R2 WHERE R1.workspaceid = R2.workspaceid ORDER BY crowdednessStatus ASC")).rows
